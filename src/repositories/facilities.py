...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def set_room_facilities(self, room_id: int, facilities_ids: list[int]) -> None:
        ...
        ids_to_delete: list[int] = list(set(current_facilities_ids) - set(facilities_ids)) # {1, 2} - {2, 3} = {1}
        ids_to_insert: list[int] = list(set(facilities_ids) - set(current_facilities_ids)) #{2, 3} - {1, 2} = {3}
